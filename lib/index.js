import { platform } from 'os';
import { start as _start, stop as _stop } from './pfctl.js';
import { start as __start, stop as __stop } from './tc.js';
import { start as ___start, stop as ___stop } from './localHostTc.js';
import { start as ____start, stop as ____stop } from './localHostPfctl.js';

function verify(options) {
  if (options.localhost) {
    if (!Number.isInteger(options.rtt)) {
      throw new Error('You need to set rtt as an integer for localhost');
    }
  } else if (
    (options.up && !Number.isInteger(options.up)) ||
    (options.down && !Number.isInteger(options.down)) ||
    (options.rtt && !Number.isInteger(options.rtt))
  ) {
    throw new Error('Input values needs to be integers');
  } else if (!options.up && !options.down && !options.rtt && !options.stop) {
    throw new Error('You need to at least set one of up/down/rtt.');
  }
}

export async function start(options = {}) {
  verify(options);

  switch (platform()) {
    case 'darwin': {
      if (options.localhost) {
        return ____start(options.rtt);
      }
      return _start(options.up, options.down, options.rtt, options.packetLoss);
    }

    case 'linux': {
      if (options.localhost) {
        return ___start(options.rtt);
      } else {
        return __start(
          options.up,
          options.down,
          options.rtt,
          options.packetLoss
        );
      }
    }

    default:
      throw new Error('Platform ' + platform() + ' not supported');
  }
}
export async function stop(options = {}) {
  switch (platform()) {
    case 'darwin': {
      if (options.localhost) {
        return ____stop();
      } else {
        return _stop();
      }
    }

    case 'linux': {
      if (options.localhost) {
        return ___stop();
      } else {
        return __stop();
      }
    }

    default:
      throw new Error('Platform ' + platform() + ' not supported');
  }
}
